...
if (count($_POST) > 0) {
    $result = mysqli_query($conn, "SELECT password from studentdb WHERE rollno='" . $_SESSION["id"] . "'");
    $row = mysqli_fetch_array($result);
	
    
    if ($_POST["currentPassword"] == $row["password"] && $_POST["newPassword"] == $_POST["confirmPassword"]) {
        mysqli_query($conn, "UPDATE studentdb set password='" . $_POST["newPassword"] . "' WHERE rollno='" . $_SESSION["id"] . "'");
