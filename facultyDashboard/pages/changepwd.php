...
if (count($_POST) > 0) {
    $result = mysqli_query($conn, "SELECT password from facultydb WHERE facultyName='" . $_SESSION["user"] . "'");
    $row = mysqli_fetch_array($result);
	
    
    if ($_POST["currentPassword"] == $row["password"] && $_POST["newPassword"] == $_POST["confirmPassword"]) {
        mysqli_query($conn, "UPDATE facultydb set password='" . $_POST["newPassword"] . "' WHERE facultyName='" . $_SESSION["user"] . "'");
